...
class MainWindow(QMainWindow):
    def __init__(self, data_provider, portfolio_manager, predictor, initial_capital, tickers):
        ...
        button_defs = [
            ...
            ("Análise Estatística", self.show_statistics),
            ("Importância Features", self.show_feature_importances),
            ("Análise Backtest", self.show_backtest_analysis),
            ("Análise de Trades", self.show_trade_analysis),
            ("Mapa Parâmetros", self.show_heatmap_optimization),
            ("Análise de Risco", self.show_risk_analysis),
            ("Atribuição PnL", self.show_pnl_attribution),
            ("Atribuição Factores", self.show_factor_attribution),
            ("Mapa Stop/Take", self.show_stop_loss_take_profit_optimization),
            ("Planeamento IA", self.show_planning_graph),
        ]
        ...
        # Coloca o botão de ajuda na última posição da grelha
        row = len(button_defs) // max_cols
        col = len(button_defs) % max_cols
