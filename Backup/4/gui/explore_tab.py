...
class ExploreTab(QWidget):
    ...
    def mostrar_detalhe_acao(self, ticker):
        ...
        if linha.empty:
            ...
            return
        # Construir results_dict para cada modelo/horizonte
        results_dict = {}
        for model in ["Logistic", "RF", "MLP"]:
            results_dict[model] = {}
            for n_ahead in [1, 3]:
                # Proba pode ser string "[x, y]", converte!
                prob = linha.iloc[0].get(f"proba_{model}_{n_ahead}d", [float('nan'), float('nan')])
                if isinstance(prob, str):
                    try:
                        import ast
                        prob = ast.literal_eval(prob)
                    except Exception:
                        prob = [float('nan'), float('nan')]
                results_dict[model][f"direction_{n_ahead}d"] = linha.iloc[0].get(f"direction_{model}_{n_ahead}d", None)
                results_dict[model][f"proba_{n_ahead}d"] = prob
                results_dict[model][f"price_pred_{n_ahead}d"] = linha.iloc[0].get(f"price_pred_{model}_{n_ahead}d", float('nan'))
                # Features podem ser string; tenta converter
                features = linha.iloc[0].get(f"features_{model}_{n_ahead}d", {})
                if isinstance(features, str):
                    try:
                        ...
                        features = {}
                results_dict[model][f"features_{n_ahead}d"] = features
